...
/**
 * @file
 * Hooks provided by the Commerce French Donate module.
 */

/**
 * Implements hook_commerce_french_donate_ajax_donation_type_refresh_alter().
 *
 * Alter the commerce_french_donate_ajax_donation_type_refresh
 * function to add ajax_command affecting other content.
 *
 * @param &$commands
 ...
 */
function hook_theme_commerce_french_donate_ajax_donation_type_refresh_alter(&$commands, $context) {
  // Check if there is values in the form_state array.
  $values = $context['form_state']['values'];
  if (empty($values)) {
    $donation_type = 'don_normal';
  }
  elseif (isset($values['commerce_french_donate'])) {
    $donation_type = $values['commerce_french_donate']['donation_type'] ? $values['commerce_french_donate']['donation_type'] : 'don_normal';
  }
  else {
